...
APP_PATH ?= 
FIRMWARE_PATH = firmware/arduino_core_eys3d
IPACK_PATH = ipack
OUT_PATH = out
SECURE_TOOL_PATH = $(TOPDIR)/$(BUILD_PATH)/tools/secure_ecv5546/secure
...
fip: check
	...
	@$(MAKE) -f $(FIP_PATH)/ecv5546.mk CROSS=$(CROSS_ARM64_COMPILE) build ;
...
config: init
	@$(MAKE) __config
	@if [ -x /usr/local/bin/eys3d-config-ext ]; then \
		/usr/local/bin/eys3d-config-ext; \
	fi
...
secure:
	@if [ "$(SECURE_PATH)" = "xboot" ]; then \
		...
		[ "$(SECURE)" = "1" ] && make -C $(TOPDIR)/build/tools/secure_ecv5546 sign IMG=$(TOPDIR)/$(UBOOT_PATH)/u-boot.bin SB=$(SB_FLAG); \
		...
		[ "$(SECURE)" = "1" ] && make -C $(TOPDIR)/build/tools/secure_ecv5546 sign IMG=$(TOPDIR)/$(FIP_PATH)/build/fip.bin SB=$(SB_FLAG); \
		...
		[ "$(SECURE)" = "1" ] && make -C $(TOPDIR)/build/tools/secure_ecv5546 sign IMG=$(TOPDIR)/$(LINUX_PATH)/arch/$(ARCH)/boot/Image.gz; \
...
init:
	...
	@$(RM) -f $(CONFIG_ROOT)
	@echo "Running config with: $(Device_Board) $(Boot_Device) $(Rootfs_Config) $(Secure_Mode) $(MT_Mode)"
	@UBUNTU_PREBUILD_URL=$$(/usr/local/bin/eys3d-config-ext prebuild_url 2>/dev/null); \
	UBUNTU_PREBUILD_URL=$$UBUNTU_PREBUILD_URL ./build/config.sh "$(Device_Board)" "$(Boot_Device)" "$(Rootfs_Config)" "$(Secure_Mode)" "$(MT_Mode)"
