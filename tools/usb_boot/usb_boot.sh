...
# 5. Copy 'rc.usbboot' to '/etc/init.d' of root partition.
...
OUTPATH=$TOP/out/boot2linux_usb
FAT_FILE_IN=$OUTPATH
ROOT_DIR_IN=$TOP/linux/rootfs/initramfs/disk
ROOT_IMG=$OUTPATH/../rootfs.img
OUT_FILE=$OUTPATH/ISP_USB_BOOOT.img
FAT_IMG_OUT=fat.img
EXT_ENV=uEnv.txt
EXT_ENV_eCV5546=uEnv_ecv5546.txt
RC_USBBOOTDIR=$ROOT_DIR_IN/etc/init.d
RC_USBBOOTFILE=rc.usbboot
FIP_IMG=fip.img
...
# cp uEnv.txt to out/usbboot
cp $EXT_ENV_eCV5546 $OUTPATH/$EXT_ENV
...
if [ -x "$(command -v mcopy)" ]; then
	...
	mcopy -i "$FAT_IMG_OUT" -s "$FAT_FILE_IN/ISPBOOOT.BIN" "$OUTPATH/$EXT_ENV"  "$OUTPATH/$FIP_IMG"  "$FAT_FILE_IN/uImage" "$FAT_FILE_IN/u-boot.img" "$FAT_FILE_IN/dtb" ::
	if [ $? -ne 0 ]; then
		exit
	fi
else
...
# Copy 'rc.usbboot' to '/etc/init.d' of root partition.
cp -rf "$RC_USBBOOTFILE" $RC_USBBOOTDIR
